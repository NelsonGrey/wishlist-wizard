...
export async function getSupportedPlatforms(req: Request, res: Response) {
  try {
    ...
    res.status(500).json({ message: "Error getting supported platforms", error: getErrorMessage(error) });
  }
}
...
export async function searchProducts(req: Request, res: Response) {
  try {
    ...
    if (errorMessage === 'No enabled e-commerce platforms to search') {
    ...
    res.status(500).json({ message: "Error searching products", error: errorMessage });
  }
}
