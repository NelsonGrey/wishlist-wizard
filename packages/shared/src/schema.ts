...
export const arModels = pgTable("ar_models", {
  id: serial("id").primaryKey(),
  productItemId: integer("product_item_id").references(() => wishlistItems.id),
  modelUrl: text("model_url").notNull(),
  thumbnailUrl: text("thumbnail_url"),
  modelType: text("model_type").notNull(), // 'glb', 'usdz', etc.
  dimensions: jsonb("dimensions"), // { width, height, depth }
  scale: decimal("scale", { precision: 10, scale: 4 }).default("1.0"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull()
});

// AR Sessions table for tracking AR usage
export const arSessions = pgTable("ar_sessions", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").notNull().references(() => users.id),
  productItemId: integer("product_item_id").references(() => wishlistItems.id),
  sessionType: text("session_type").notNull(), // 'room_view', 'size_compare', etc.
  duration: integer("duration"), // in seconds
  interactions: jsonb("interactions"), // JSON log of user interactions
  createdAt: timestamp("created_at").defaultNow().notNull(),
  endedAt: timestamp("ended_at")
});

// Barcode scan logs for analytics
export const barcodeScanLogs = pgTable("barcode_scan_logs", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id),
  barcode: text("barcode").notNull(),
  found: boolean("found").notNull().default(false),
  matchType: text("match_type"), // 'exact', 'similar', 'none'
  productData: jsonb("product_data"), // Store the found product info
  scannedAt: timestamp("scanned_at").defaultNow().notNull()
});

// Relations for the new AR and barcode tables
export const arModelsRelations = relations(arModels, ({ one }) => ({
  productItem: one(wishlistItems, {
    fields: [arModels.productItemId],
    references: [wishlistItems.id]
  })
}));

export const arSessionsRelations = relations(arSessions, ({ one }) => ({
  user: one(users, {
    fields: [arSessions.userId],
    references: [users.id]
  }),
  productItem: one(wishlistItems, {
    fields: [arSessions.productItemId],
    references: [wishlistItems.id]
  })
}));
...
// Schema exports for the new tables
...
export const insertArSessionSchema = createInsertSchema(arSessions).omit({
  id: true,
  createdAt: true
});
