...
function getFirebaseFunctionRoute(url: string, method: string, body?: unknown): FunctionRouteMatch {
  ...
  const patterns: Array<{ pattern: RegExp; resolve: (match: RegExpExecArray) => FunctionRouteMatch }> = [
    ...
    { pattern: /^\/api\/analytics\/ad-revenue-summary$/, resolve: () => ({ functionName: 'getAdRevenueSummary', data }) },
    { pattern: /^\/api\/analytics\/ad-kpi-snapshot$/, resolve: () => ({ functionName: 'createAdKpiSnapshot', data }) },
    { pattern: /^\/api\/analytics\/ad-kpi-snapshots$/, resolve: () => ({ functionName: 'getAdKpiSnapshots', data }) },
