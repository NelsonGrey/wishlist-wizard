...
**Status**: 🟡 Pre-launch blocker — Create Wishlist confirmed broken in `wishlist-wizard-dev` and `wishlist-wizard-staging`; the authenticated `/app/*` route block in `packages/web/client-src/AppRouter.tsx` was found accidentally gated out of production by an unrelated cross-project automation commit (2026-05-06); the gate was removed 2026-07-16 and production now matches dev/staging. However, a 2026-07-18 audit (`docs/WISHLIST_WIZARD_GO_LIVE.md` §1.15) found the `createWishlist` Cloud Function unconditionally enforces Firebase App Check while the web client never initializes it — confirmed independently via plain `curl` calls (real auth token, no browser involved) against both dev and staging, both rejected identically. Production wasn't tested (still behind a "Coming Soon" gate, so no active users affected yet) but almost certainly has the same issue since it shares the same codebase/deploy. Must be fixed before the production gate comes down.
