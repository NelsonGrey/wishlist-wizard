...
- **CI is red on `develop`:** "Deploy Web App to Dev (Hosting)" has failed every push since ~2026-09-02 — the legacy `firebase-hosting-dev.yml` still authenticates with `FIREBASE_TOKEN`, which has **expired again** (`credentials are no longer valid ... firebase login --reauth`). Dev-site hosting deploys only; the WIF-based main pipeline is unaffected. Left un-fixed deliberately (pre-launch CI-green is not the priority) — needs a token refresh or a migration of that one workflow to WIF.
