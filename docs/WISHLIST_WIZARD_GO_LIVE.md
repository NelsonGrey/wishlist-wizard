...
- **`develop` is ~33 commits ahead of `main`/`staging` (unpromoted ~2 weeks)** and now includes a large mobile pass: 7 web→mobile parity features (reserve item, wishlist CSV export, account data-export/revoke-sessions/contact-support, default privacy settings screen, edit-profile + avatar upload, in-app shared-wishlist viewer, add-item store/priority), a design-system alignment to the website palette + brand logo + launcher icon, a rebuilt Profile screen, the compact Home greeting + defined ad-slot separator, and **removal of the contact-sales "Enterprise" tier from the mobile Subscription screen** (App Store / Play Store policy — no non-store purchase option). `flutter test` 370 green, all 11 mobile integration flow suites green (3 integration-harness bugs found and fixed in the process).
- **CI: "Deploy Web App to Dev (Hosting)" fixed 2026-09-02.** It had failed every push to `develop` because `firebase-hosting-dev.yml` still used `firebase deploy --token` — and Google has stopped honouring `firebase login:ci` tokens entirely (a **refreshed** token fails the identical "credentials are no longer valid" way). `master-pipeline.yml` only runs on push to `staging`/`main`, so `develop` genuinely needed this standalone workflow. Migrated it to the same Workload Identity Federation auth `firebase-deploy-local.yml` already uses for staging/main hosting (`google-github-actions/auth@v3` + `github-actions-deploy@wishlist-wizard-dev` + live Firebase Web-config fetch, no `--token`). Verified green: run `33701000184` → `✔ hosting[wishlist-wizard-dev]: release complete`. `FIREBASE_TOKEN` is now unused anywhere.

**Important:** Part 2.3's automated code-quality gates were re-run 2026-09-02 (above); the two smoke tests and `go-live-gate.sh` still cannot go green until the companion-repo smoke script is updated for the router migration. The three "code" gates (TS, lint, requirements) are current and green.

**Update 2026-08-24/25:** Firebase App Check (§1.15–§1.20 below) was **removed entirely** — web, iOS, and backend enforcement all gone. It kept causing more operational friction (headless-browser reCAPTCHA failures blocking CI/deploys) than the protection was worth, matching the same call already made on this org's other two projects. Everything below describing App Check as configured/enforced is historical record of a mechanism that no longer exists — don't use it to reason about current security posture. Separately, `develop` had drifted ~150 commits ahead of `staging`/`main` since ~2026-08-10 and has since been substantially re-synced, and Firebase Hosting deploys are now fully automated for the first time (previously only Functions/Firestore deployed automatically). This status table otherwise still reflects 2026-08-10 and should not be trusted for anything not called out here without a fresh check.
...
Promotion path: `develop` → `main` → production.
...
**Update 2026-08-25:** `firebase-hosting-merge.yml`/`firebase-hosting-staging.yml` (the mechanism described above) were retired — they'd gone stale again (expired `FIREBASE_TOKEN`) and were fully redundant once `master-pipeline.yml`'s `firebase-deploy-v2` job started deploying Hosting itself (via `firebase-deploy-local.yml`, using the same WIF credentials already used for Functions). `develop` still deploys separately via `firebase-hosting-dev.yml` on push to `develop`, unchanged.
...
**Full §2.3 sequence re-run 2026-09-02 (first end-to-end pass since 2026-07-16).** Summary: the three "code" gates (TS, lint, requirements) are green; every red gate traces to stale local test tooling or an un-built release artifact, **not to a product defect** — the deployed dev backend answers fine (verified separately by the mobile integration suite the same day). Details per line below.

- [ ] `npm audit --audit-level=high` — **regressed to 13 vulns (3 high, 10 moderate)** vs. "0 high" on 2026-07-16. All 3 high are transitive in **dev/build tooling**, not the production runtime: `brace-expansion` (via eslint / glob / rimraf — DoS), `browserslist` (via the Vite web build — OOM/crash on untrusted stats json), `nanoid` (infinite loop only when `size:0`, which nothing in this codebase does). `npm audit fix` (non-`--force`) would clear them, **but that regenerates the root `package-lock.json`** — the exact file whose drift against the private `wishlist-wizard-functions` checkout has broken CI `npm ci` twice (§1.17, §1.20), and that companion checkout currently has its own uncommitted `package.json`/lock changes. Deferred pending a deliberate lockfile-refresh window. — Owner: _______
- [x] `npm run check` — TypeScript passes with 0 errors across web/shared/firebase-utils — Re-verified 2026-09-02
- [x] `npm run lint` — ESLint passes with 0 errors across web/shared/firebase-utils — Re-verified 2026-09-02
- [x] Secret scan (gitleaks, proper git-history mode) — 2 false positives (placeholder token in docs, fake test Stripe key) + 1 real historical finding, see §1.9 (acknowledged, not remediated by rewrite — main/production unaffected) — Verified 2026-07-16 (Secret Scan CI job still green on `develop` as of 2026-09-02)
- [ ] `npm run test:functions:smoke:all:strict` / `npm run test:users:smoke` — **both now run** (the local `packages/functions` companion checkout is present and the emulator boots), but **both fail at fixture setup: `createUserProfile fixture setup failed: 404`.** Root cause is a **stale smoke script**, not a backend regression: the script (`packages/functions/scripts/emulator-smoke-all-functions.cjs:309`, plus the `case 'createUserProfile'/'getUserProfile'/'updateUserProfile'` blocks at ~4332) calls raw callables that the 2026-07-23 router migration replaced with `ensureMyProfile`/`getMyProfile`/`updateMyProfile` routed at `POST/GET/PATCH /api/profile`. Fix belongs in the private companion repo (`wishlist-wizard-functions`), which currently has unrelated uncommitted WIP in its tree — not touched this pass. — Owner: _______
- [x] `npm run requirements:traceability` — passes (14 business / 18 technical requirements, 8 work packages, 0 warnings, 0 failures) — Re-verified 2026-09-02
- [x] `npm run requirements:verify` — passes ("phased mode"; enforced-drift 0, enforced-without-design-linkage 0; ~14 completed items still "unmapped, non-blocking in phased mode") — Re-verified 2026-09-02
- [ ] `./scripts/go-live-gate.sh` — **RELEASE BLOCKED, 4 blocking issues, 14 passed / 2 warnings** (2026-09-02). All 4 blockers are downstream of the two stale smoke tests above — CHECK 2 "Firebase endpoints: 1 failed / 0-of-0 ready (need ≥60)" and CHECK 3 "Core user flows: 1 failed" both read `artifacts/smoke-*-report.json`, which the failing smoke runs just overwrote with their fixture-setup error; CHECK 5 "Tier 1 features: 0/10 validated" derives from the same empty endpoint set. CHECK 9 (TypeScript) and CHECK 11 (requirements + design linkage) pass. Warnings: uncommitted changes in the tree (concurrent WIP) + limited Tier 2 features. **This gate cannot go green until the companion-repo smoke script is updated.** — Owner: _______
- [ ] `npm run preflight:extension` — **fails: "Missing packaged extension archive (*.tgz) in dist/"** — a precondition, not a defect: the extension hasn't been packaged this pass. Run the extension package step first, then re-run. — Owner: _______
- [ ] `npm run preflight:mobile` — **fails: "Missing Android release artifact ... app-release.apk"** — precondition, not a defect. Steps 1–3 *do* pass: no tracked sensitive key files, **no "coming soon" placeholder strings in app code**, Flutter version metadata present. Build a release APK, then re-run. — Owner: _______
