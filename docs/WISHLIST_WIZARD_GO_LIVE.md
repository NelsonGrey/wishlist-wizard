...
|---|---|---|
| **Web app** | 🟡 Pre-launch blocker: create-wishlist broken in dev + staging (prod not launched yet, so no active users affected) | Production gate bug (§1.7) fixed — `/app/*` and `/shared/:shareId` now render in prod, matching dev/staging. **2026-07-18 (§1.15): `createWishlist` confirmed broken in both `wishlist-wizard-dev` and `wishlist-wizard-staging`** — Firebase App Check is unconditionally enforced on the callable function (proven via plain `curl` calls, no browser involved) but the web client never initializes App Check anywhere, so every real caller is rejected with a 403. Production was not tested (still gated behind a "Coming Soon" page, per `VITE_SHOW_COMING_SOON_PRODUCTION` — no real users to affect yet), but shares the same codebase/function deploy, so treat it as very likely affected too until fixed. **Must be fixed before the "Coming Soon" gate comes down.** Two other real bugs found and fixed the same day: a shared-wishlist privacy check that always evaluated visitors as anonymous, and a registration form that rejected leaving its "(optional)" display name blank. Add item (incl. paste-a-link auto-fetch) and share were code-reviewed clean. |
...
Production (`wishlist-wizard-prod`) was **not tested** — the user confirmed it's still gated behind a "Coming Soon" page (`VITE_SHOW_COMING_SOON_PRODUCTION`), so there are no real users there to be affected yet, and a live test was correctly blocked by the session's own safety guardrails as too risky to run unprompted against production. Given prod runs the same codebase and (presumably) the same function deploy as dev/staging, treat it as very likely affected too. **This must be fixed before the "Coming Soon" gate comes down** — otherwise the very first real users would hit this immediately.

- [ ] **Either wire up the App Check SDK client-side (with a registered debug token for local/E2E testing) or relax enforcement on affected callables, then verify in all three environments** — Owner: _______ — must land before removing the production "Coming Soon" gate
