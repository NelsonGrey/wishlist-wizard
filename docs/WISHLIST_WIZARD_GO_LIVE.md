...
|---|---|---|
| **Web app** | 🔴 Create-wishlist confirmed broken in dev; staging/prod unverified | Production gate bug (§1.7) fixed — `/app/*` and `/shared/:shareId` now render in prod, matching dev/staging. **2026-07-18 (§1.15): `createWishlist` is confirmed broken in `wishlist-wizard-dev` — Firebase App Check is unconditionally enforced on the callable function (proven via a plain `curl` call, no browser involved) but the web client never initializes App Check anywhere, so every real caller is rejected with a 403. Whether staging/production have the same enforcement has not been checked.** Two other real bugs found and fixed the same day: a shared-wishlist privacy check that always evaluated visitors as anonymous, and a registration form that rejected leaving its "(optional)" display name blank. Add item (incl. paste-a-link auto-fetch) and share were code-reviewed clean. |
...
### 1.15 Web App: Same Rigor Pass — Two Real Bugs, a Severe App Check Blocker, and an E2E Suite That Had Never Really Passed [PARTIALLY RESOLVED — 2026-07-18]

Applied the same audit-then-verify pass to the web app (`packages/web`). This one surfaced the most severe finding of the whole recovery effort.
...
**The severe, still-open finding: Firebase App Check is unconditionally enforced on `createWishlist` (and likely other callable functions) in `wishlist-wizard-dev`, and the web client never initializes App Check anywhere.** Confirmed with a plain `curl` call — a real ID token, obtained directly from the Identity Toolkit REST API, with zero browser/Playwright/automation involved — against the deployed function, which still returned `403 Request failed App Check verification. Please ensure you are using the official app.` This is not a bot-detection false positive; it's the same rejection any caller gets without a valid App Check token, including the real web app itself. **As currently configured, no real user can create a wishlist through the web app in `wishlist-wizard-dev`.**

- [ ] **Confirm whether staging/production have the same App Check enforcement, and either wire up the App Check SDK client-side or relax enforcement on affected callables** — Owner: _______ (not verified against staging/prod: doing so would mean creating real accounts/data in those environments, which wasn't done without your sign-off; this needs to be checked and fixed before treating web wishlist creation as reliable anywhere)
